...
fn build_mcu_modules(
    ...
    for (mcu, inputs) in mcus {
        ...
        let includer_path = gen_dirs.patch.join(mcu).with_extension("yaml");
...
- {}
"#,
                    unpatched_svd_path.display(),
                    inputs.patch_path.display()
                );
                let Ok(()) = fs::write(&includer_path, &includer_content) else {
                    println!("cargo::error=Failed to write includer file for {}!", mcu);
                    return Err(());
                };
                let Ok(()) = svdtools::patch::process_file(
                    &includer_path,
                ...
                svd_content = fs::read_to_string(&patched_svd_path);
        ...
        svd2rust_config.output_dir = gen_dirs.module.clone();
